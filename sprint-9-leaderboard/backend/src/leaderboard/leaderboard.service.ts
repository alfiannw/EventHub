...
// Initial Mock Participants mirroring EventHub Main Registry
...
export class LeaderboardService {
  ...
  async getLeaderboard(search?: string, filterCompany?: string, filterTier?: string): Promise<LeaderboardEntryDto[]> {
    ...
    // Filter by Search (Name / Email / Position)
    if (search) {
    ...
    if (filterTier) {
      list = list.filter(p => {
        ...
        return p.points < 5; // UNRANKED / PENDING
      });
    }

    // Sort by Points descending, then checkedIn, then Name ascending
    list.sort((a, b) => {
    ...
    // Compile into DTO including Ranks and Unlocked Milestones
  ...
  async adjustScore(dto: AdjustScoreDto, actor: string = 'SYSTEM_DESK'): Promise<ScoreLogEntity> {
    ...
    // Automatically trigger check-in boarding flag if check_in point awarded
  ...
  private async evaluateMilestones(participantId: string, currentTotal: number) {
    ...
    // Helper to add milestone if missing
    ...
    // Helper to remove milestone if score dropped below threshold (Manual negative corrections)
  ...
  async claimMilestone(dto: ClaimMilestoneDto, actor: string = 'REGISTRATION_DESK'): Promise<MilestoneEntity> {
    ...
    if (!milestone) {
      throw new NotFoundException(`Milestone reward "${dto.milestoneName}" is currently locked for this participant.`);
    }

    if (milestone.claimed) {
      throw new BadRequestException('Milestone reward has already been claimed.');
    }
  ...
  async resetLeaderboard() {
    this.participants.forEach(p => {
      p.points = p.id === 'p-5' ? 0 : 5; // Clear or minimize points
