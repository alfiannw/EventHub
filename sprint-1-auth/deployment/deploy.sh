...
echo "========================================="
echo "   STARTING EVENTHUB SPRINT 1 DEPLOY     "
echo "========================================="
...
docker build \
  -t ${REGION}-docker.pkg.dev/${PROJECT_ID}/${GAR_REPO}/${SERVICE_NAME_BACKEND}:sprint1 \
...
docker build \
  -t ${REGION}-docker.pkg.dev/${PROJECT_ID}/${GAR_REPO}/${SERVICE_NAME_FRONTEND}:sprint1 \
  -f ../docker/Dockerfile.frontend \
  ../frontend

# 4. Push Containers to Artifact Registry
echo "Pushing images to Artifact Registry..."
docker push ${REGION}-docker.pkg.dev/${PROJECT_ID}/${GAR_REPO}/${SERVICE_NAME_BACKEND}:sprint1
docker push ${REGION}-docker.pkg.dev/${PROJECT_ID}/${GAR_REPO}/${SERVICE_NAME_FRONTEND}:sprint1

# 5. Deploy Backend Server to Cloud Run
echo "Deploying backend server to Google Cloud Run..."
gcloud run deploy ${SERVICE_NAME_BACKEND} \
  --image ${REGION}-docker.pkg.dev/${PROJECT_ID}/${GAR_REPO}/${SERVICE_NAME_BACKEND}:sprint1 \
  --platform managed \
  --region ${REGION} \
...
echo "Deploying frontend client to Google Cloud Run..."
gcloud run deploy ${SERVICE_NAME_FRONTEND} \
  --image ${REGION}-docker.pkg.dev/${PROJECT_ID}/${GAR_REPO}/${SERVICE_NAME_FRONTEND}:sprint1 \
  --platform managed \
  --region ${REGION} \
  --allow-unauthenticated \
...
echo "========================================="
echo "   DEPLOY SUCCESSFUL FOR SPRINT 1!       "
echo "========================================="
